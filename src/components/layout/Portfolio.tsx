    ...
    <section id="portfolio" className="relative py-20 sm:py-32">
      ...
      <div className="container-custom">
        <div className="flex items-center justify-center gap-3 mb-5 sm:mb-6">
          ...
          <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.3em] sm:tracking-[0.35em] text-gold-300">Selected Work</span>
          <span className="h-px w-8 sm:w-10 bg-gold-400/40" />
        </div>
        ...
        <motion.h2
          ...
          viewport={{ once: true }}
          className="text-3xl sm:text-4xl md:text-7xl font-bold mb-12 sm:mb-20 text-center gold-gradient-text"
        >
          ...
          {(dict.portfolio.items as PortfolioItem[]).map((item, index) => (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row">
                {/* Visual */}
                <div className="relative w-full md:w-3/5 h-56 sm:h-72 md:h-[28rem] overflow-hidden bg-ink-800">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-4xl sm:text-5xl md:text-7xl font-black gold-gradient-text opacity-20 group-hover:opacity-40 transition-opacity duration-500 tracking-tight">
                      3D MAP
                    </span>
                  </div>
                  <div className="absolute top-4 left-4 sm:top-6 sm:left-6 text-[10px] sm:text-[11px] uppercase tracking-[0.25em] sm:tracking-[0.3em] text-gold-300/90">
                    Case 01
                  </div>
                </div>

                {/* Content */}
                <div className="w-full md:w-2/5 p-6 sm:p-8 md:p-12 flex flex-col justify-center">
                  <span className="text-[11px] sm:text-xs uppercase tracking-[0.25em] text-gold-300 mb-4 sm:mb-5">
                    {item.category}
                  </span>
                  <h3 className="text-xl sm:text-2xl md:text-4xl font-bold mb-4 sm:mb-5 text-neutral-50 leading-tight group-hover:text-gold-100 transition-colors">
                    {item.title}
                  </h3>
