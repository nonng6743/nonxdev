  ...
  return (
    <>
      <motion.nav
        ...
        className="fixed top-0 left-0 right-0 z-50 flex justify-center py-4 sm:py-5 px-4 pointer-events-none"
      >
        <div
          className="pointer-events-auto flex items-center gap-3 sm:gap-6 px-4 sm:px-5 py-2.5 rounded-full
                     border border-gold-400/20 bg-ink-900/80 backdrop-blur-xl
                     shadow-[0_8px_40px_-12px_rgba(212,175,55,0.25)] max-w-full"
        >
          <Link href="/" className="font-display font-bold text-lg tracking-tight flex items-baseline gap-0.5 shrink-0">
          ...
          {/* Desktop nav */}
          <div className="hidden md:flex items-center gap-5 text-xs uppercase tracking-[0.18em] text-neutral-400 pl-4 border-l border-gold-400/20">
            <div
              ref={servicesRef}
              className="relative"
            ...
            <a href="/#portfolio" className="hover:text-gold-200 transition-colors">
              Work
            </a>
            <a href="/#contact" className="hover:text-gold-200 transition-colors">
              Contact
            </a>
          </div>

          {/* Desktop language */}
          <div className="hidden md:flex items-center gap-1.5 text-[11px] font-medium tracking-wider pl-4 border-l border-gold-400/20">
            <LangButton active={language === 'en'} onClick={() => setLanguage('en')} label="EN" />
            <span className="text-neutral-700">·</span>
            <LangButton active={language === 'th'} onClick={() => setLanguage('th')} label="TH" />
          </div>
          ...
          <button
            ...
            className="md:hidden ml-1 w-9 h-9 inline-flex items-center justify-center rounded-full text-neutral-300 hover:text-gold-200 transition-colors"
          >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 md:hidden bg-ink-900/95 backdrop-blur-xl overflow-y-auto"
          >
            <div className="pt-24 pb-12 px-6 max-w-md mx-auto">
              <MobileSection label={dict.services.products.eyebrow}>
                {(dict.services.products.items as DropdownItem[]).map((item, i) => (
                  <MobileLink
                    key={i}
                    icon={productIcons[i % productIcons.length] ?? Sparkles}
                    title={item.title}
              ...
              <MobileSection label={dict.services.agency.eyebrow}>
                {(dict.services.agency.items as DropdownItem[]).map((item, i) => (
                  <MobileLink
                    key={i}
                    icon={agencyIcons[i % agencyIcons.length] ?? Sparkles}
                    title={item.title}
                    desc={item.description}
                    href={item.href ?? '/#agency'}
                    onClick={() => setMobileOpen(false)}
                    external={!!item.href}
                  />
                ))}
              ...
              <MobileSection label="Navigate">
                <Link
                  href="/#portfolio"
                  onClick={() => setMobileOpen(false)}
                  className="block px-4 py-3 rounded-xl hover:bg-ink-800/80 text-neutral-100"
                >
                  Work
                </Link>
                <Link
              ...
              <div className="pt-6 mt-2 border-t border-gold-400/10 flex items-center justify-between">
                <span className="text-[11px] uppercase tracking-[0.3em] text-gold-300/70">Language</span>
  ...
  return (
    <button
      ...
      className={`px-1.5 py-0.5 rounded transition-colors ${
        active ? 'text-gold-200' : 'text-neutral-500 hover:text-neutral-300'
      }`}
    >
      {label}
    </button>
...
function MobileSection({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="mb-6">
      <div className="text-[10px] uppercase tracking-[0.3em] text-gold-300/70 mb-2 px-2">{label}</div>
...
function MobileLink({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className="flex items-start gap-3 px-3 py-3 rounded-xl hover:bg-ink-800/80 transition-colors"
    >
      <span className="mt-0.5 inline-flex w-9 h-9 shrink-0 items-center justify-center rounded-lg border border-gold-400/25 bg-ink-800 text-gold-300">
        <Icon className="w-4 h-4" />
      </span>
      <span className="flex-1 min-w-0">
        <span className="flex items-center gap-1.5 text-sm font-semibold text-neutral-100">
          <span className="truncate">{title}</span>
          {external && <ArrowUpRight className="w-3.5 h-3.5 shrink-0 text-gold-300/80" />}
        </span>
        <span className="block mt-0.5 text-[11.5px] leading-snug text-neutral-400 line-clamp-2">{desc}</span>
      </span>
    </Link>
  );
}
